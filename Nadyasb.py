...
oepoll = OEPoll(nadya)
admin=['ub5abe828cd964292195c3c59d6322033',nadyaMID]
#==============================================================================#
...
def Rapid1Say(mtosay):
    nadya.sendText(Rapid1To,mtosay)
...
#==============================================================================#
                    ...
                    nadya.sendMessage(to, "Berhasil mengaktifkan Auto Block")
                elif text.lower() == 'autoblock off':
                    settings["autoBlock"] = False
                    nadya.sendMessage(to, "Berhasil menonaktifkan Auto Block")
...
#==============================================================================#
                    ...
                    nadya.sendImageWithURL(msg.to, cover)
                        ...
                        if settings["Wc"] == True:
                            if settings["lang"] == "JP":
                                nadya.sendMessage(to,"เปิดข้อความต้อนรับเมื่อมีสมาชิกเข้ากลุ่ม   ")
                        else:
                            settings["Wc"] = True
                            if settings["lang"] == "JP":
                                nadya.sendMessage(to,"เปิดข้อความต้อนรับเมื่อมีสมาชิกเข้ากลุ่ม   ")
                        ...
                        if settings["Wc"] == False:
                            if settings["lang"] == "JP":
                                nadya.sendMessage(to,"ปิดข้อความต้อนรับเมื่อมีสมาชิกเข้ากลุ่ม   ")
                        else:
                            settings["Wc"] = False
                            if settings["lang"] == "JP":
                                nadya.sendMessage(to,"ปิดข้อความต้อนรับเมื่อมีสมาชิกเข้ากลุ่ม   ")
                
                elif msg.text.lower() == 'leve on':
                        if settings["Lv"] == True:
                            if settings["lang"] == "JP":
                                nadya.sendMessage(to,"เปิดข้อความอำลาเมื่อมีสมาชิกออกกลุ่ม   ")
                        else:
                            settings["Lv"] = True
                            if settings["lang"] == "JP":
                                nadya.sendMessage(to,"เปิดข้อความอำลาเมื่อมีสมาชิกออกกลุ่ม   ")
                elif msg.text.lower() == 'leve off':
                        if settings["Lv"] == False:
                            if settings["lang"] == "JP":
                                nadya.sendMessage(to,"ปิดข้อความอำลาเมื่อมีสมาชิกออกกลุ่ม   ")
                        else:
                            settings["Lv"] = False
                            if settings["lang"] == "JP":
                                nadya.sendMessage(to,"ปิดข้อความอำลาเมื่อมีสมาชิกออกกลุ่ม   ")

                elif text.lower() == 'rejec':
                    gid = nadya.getGroupIdsInvited()
                    start = time.time()
                    for i in gid:
                        nadya.rejectGroupInvitation(i)
                    elapsed_time = time.time() - start
                    nadya.sendMessage(to, "ลบรันเสร็จแล้วขอรับ")
                    nadya.sendMessage(to, "ระยะเวลาที่ใช้: %sวินาที" % (elapsed_time))
            ...
            if msg.toType == 0:
                ...
                if op.type == 19:
                    ...
                    if settings["Nk"] == True:
                      ...
                      dan = nadya.getContact(op.param2)
                      tgb = nadya.getGroup(op.param1)
                      nadya.sendMessage(op.param1, "เฮ้ย {}, คือหยังมันโหดแท้วะΣ(っﾟДﾟ；)っ ".format(str(dan.displayName)))
                      nadya.sendContact(op.param1, op.param2)
                      nadya.sendImageWithURL(op.param1, "http://dl.profile.line-cdn.net{}".format(dan.picturePath))
                if op.type == 15:
                    ...
                    if settings["Lv"] == True:
                      ...
                      dan = nadya.getContact(op.param2)
                      tgb = nadya.getGroup(op.param1)
                      nadya.sendMessage(op.param1," 📢 {}, ได้ออกจากกลุ่ม {} \nยืนไว้อาลัยแด่เขาเป็นเวลา3วินาที  (｀・ω・´)".format(str(dan.displayName),str(tgb.name)))
                      nadya.sendContact(op.param1, op.param2)
                      nadyaa.sendImageWithURL(op.param1, "http://dl.profile.line-cdn.net{}".format(dan.picturePath))
#==============================================================================#
        if op.type == 55:
